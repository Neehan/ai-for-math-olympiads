...
- **States:** label every nonempty baseline and hinted Self-Refine checkpoint; fit dynamics on the fitting split and test predictions held out. Audit score ≥5 gives `S`; missing text is unobserved. For other checkpoints, annotate recognition of each step in the frozen outline using its matching reference: 3/3 gives provisional `P`, otherwise `U`. Double-annotate these decisions and adjudicate materially different candidate routes before fitting the general-strategy model.
