...
| `late-baseline-sequential` / `late-hint-sequential` | Give a fresh session the same serialized failed unaided history through 3×, then Self-Refine for one new 1× without or with the oracle strategy | Matched estimate of whether accumulated failed context attenuates oracle guidance |
