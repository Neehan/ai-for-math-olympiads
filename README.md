...
| `late-baseline-sequential` / `late-hint-sequential` | Give a fresh session the same audited failed unaided 3× draft, then Self-Refine for one new 1× without or with the oracle strategy | Matched estimate of whether commitment to a failed draft attenuates oracle guidance |
