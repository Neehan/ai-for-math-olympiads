...
- **sequential** — solve → (critique → revise)× under one shared output-token budget, cut off mid-phase when the budget is spent (arms `baseline-sequential`, `hint-sequential`, `outline-sequential`). The matched late experiment first selects problems with fewer than two of three original `baseline-sequential` passes at 3×. `late-baseline-sequential` then creates a fresh exact 3× native session, retains its transcript and scratch state, and continues a no-hint fork for one 1× block. Run second, `late-hint-sequential` forks that same retained state and differs only by the ordinary h2 block. All three seeds are run, including prefixes already solved in the fresh replication; outcome conditioning is post hoc. Missing native sources are skipped, never reconstructed from text. Ordinary trajectories stop after the registered no-gap rule; the late prefix deliberately runs to its fixed 3× boundary before forking.
...
./run.sh run --arm late-baseline-sequential --model litellm/gpt-5.4 --seeds 1,2,3
./run.sh run --arm late-hint-sequential --model litellm/gpt-5.4 --seeds 1,2,3
