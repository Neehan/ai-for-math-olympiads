...
- **Prior results never enter a generation container.** The run stage mounts a staging dir pre-seeded with only the selected model/arm's `meta.json` completion markers (resume still works), then merges only newly completed attempts from that same model/arm back into `results/` after the container exits. Marker-only resume inputs are discarded rather than re-merged, preventing a long-running arm from resurrecting stale results archived while it was active. An agent can never read past arms' solutions or hint-carrying logs, even by exploring the filesystem. The audit stage mounts the real tree (the judge must read solutions).
