...
- **uniform_strategy** — one shared planner (≤80k) consolidates up to eight semantically distinct whole-proof candidate strategies, then eight fresh executors receive one assigned strategy each. `baseline-uniform-strategy` is one complete 8× bank.
...
- **Uniform Strategy Search-8:** the planner receives 5% from each executor allocation, so an 80k shared planner produces `m≤8` semantically distinct whole-proof candidate strategies and eight fresh executors receive 190k each: `80k + 8×190k = 1.6M = 8×`. Executors are allocated round-robin across strategies, differing by at most one when `m` does not divide eight. Every executor receives its assigned planner output through the exact same neutral strategy wrapper as the privileged-strategy arm: the candidate is proposed, not certified correct, and the executor must check it while proving every step. Generated strategies are not externally verified or selected before execution. Unused tokens do not transfer. Bank audit reports oracle-audited candidate coverage (any valid proof), not selected-proof accuracy.
