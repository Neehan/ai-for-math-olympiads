...
- **uniform_strategy_only / uniform_compress** — freeze the identical Uniform-C planner bank without executors, then reproducibly sample three raw entries without replacement or outcome filtering and compress them independently with GPT-5.6-sol. Existing full Uniform-C planner artifacts are copied exactly rather than regenerated. Compression is tool-free, preserves errors, and enforces ≤25 words; banks with fewer than three proposals cannot enter selection and remain reported proposal failures. The ordinary `audit` command skips proof grading for these non-proof artifacts and applies the same three-step strategy audit to their displayed text.
- **selection / selection_no_problem** — rank a frozen four-item set (three audited compressed proposals plus the exact oracle sketch) in three deterministic order randomizations under a tool-free, one-turn, 20k-output-token (0.1×) cap. `selection` uses the source model and problem statement; `selection-no-problem` requires `--worker-model` and measures oracle-style leakage without the problem. `selection-10k` and `selection-40k` are otherwise identical configured sensitivity arms with separate result/checkpoint namespaces. All store full rankings, the exact oracle rank, and the number of candidates labeled `strategy_acquired`; a cap-limited response without a valid ranking is a terminal `no_decision`, not a resumable reasoning turn.

## Configuration — `config.json`

Single source of experiment knobs: `model` (solver), `audit_model` (judge; must differ from the solver) — both overridable per invocation with `--model` / `--audit-model` (config values are the defaults; the solver≠judge check applies to the effective pair, and audit's `--model` selects whose results tree to grade), `effort` (fixed `high` per the paper), `unit_output_tokens` (1× = 200k), `selection_output_tokens` (per-selection-arm caps), the Uniform Strategy planner/reserve/branch settings, `max_turns_per_phase` / `audit_max_turns` (per-phase guards; the token budget is the sequential stop), `max_concurrency`, and the arm table (`hint`: none/h1/h2/h3, `mode`, `budget_units`, `seeds`). Models are Anthropic ids (`claude-opus-4-8`), OpenRouter `vendor/model` ids (`openai/gpt-5.5`, `deepseek/deepseek-v4-flash-0731`), local Codex-subscription aliases (`litellm/gpt-5.4`, `litellm/gpt-5.4-mini`, `litellm/gpt-5.5`, `litellm/gpt-5.6-luna`, `litellm/gpt-5.6-sol`), or native Anthropic-compatible vLLM ids (`vllm/qed-nano`, `vllm/muse-glimmer`). DeepSeek V4 Flash is restricted to Relace, Baidu Qianfan, StreamLake, and DeepInfra, sorted by live throughput and capped at $0.08/M input and $0.18/M output. Local routes use their `*_BASE_URL*` values as the round-robin/cooldown pool and a shared `*_API_KEY`; setup is in `docs/codex-subscription-via-litellm.md` and `docs/qed-nano-vllm.md`. Results paths use the model id with `/` replaced by `-`. Arm names are the slugs used everywhere — CLI, `results/` paths, and the top-level README arm table. Reliability arms use seeds 1–3; the two expensive search controls each use one top-level bank seed containing eight executor runs.
...
- `hard_hint_selection.jsonl` — frozen candidate sets for the selection arms, keyed by `source_model` and `problem_id`. Each record contains the exact oracle sketch and exactly three compressed generated strategies with a canonical boolean `strategy_acquired` label. `acquisition_basis` records `reference_steps`, `human_alternative`, or `none`; a blinded expert-accepted alternative also requires a short `adjudication_note`. The harness rejects inconsistent labels, oracle drift, missing labels, duplicate identities, or any count other than three.
...
- **Prior proof results never enter a generation container.** The run stage mounts a staging dir pre-seeded with only `meta.json` completion markers (resume still works), then merges only durable pairs containing both `solution.md` and the last-written `meta.json`. The compression arm additionally receives the exact planner-only `strategies.json` dependency and its non-proof manifest; its worker is tool-free. No prior proof, executor log, or proof scratch enters that container. For an interrupted bank, completed `run_<kk>` children are retained while marker-only and partial children are discarded. This prevents stale-result resurrection without losing paid bank members. The audit stage mounts the real tree (the judge must read solutions).
...
./run.sh run --arm baseline-uniform-strategy-only --model litellm/gpt-5.4 --problems <ids>
./run.sh audit --arm baseline-uniform-strategy-only --model litellm/gpt-5.4
./run.sh run --arm baseline-uniform-compress --model litellm/gpt-5.4 --problems <ids>
./run.sh audit --arm baseline-uniform-compress --model litellm/gpt-5.4 --problems <ids>
python scripts/build_selection_dataset.py --output local_data/hard_hint_selection.jsonl
# Before any ranking run, review the exact compressed sketches blind to ranking
# outcomes. For a verified alternative route, set strategy_acquired=true,
# acquisition_basis="human_alternative", and add a concise adjudication_note;
# otherwise retain the frozen reference_steps/none annotation. Then upload.
./run.sh run --arm selection --model litellm/gpt-5.4 --problems <ids>
./run.sh run --arm selection-10k --model litellm/gpt-5.4 --problems <ids>
./run.sh run --arm selection-40k --model litellm/gpt-5.4 --problems <ids>
./run.sh run --arm selection-no-problem --model litellm/gpt-5.4 --worker-model <external-model> --problems <ids>
./run.sh run --arm hint
