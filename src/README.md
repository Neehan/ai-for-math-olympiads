...
- **two-block sequential replication** — `baseline-sequential-2x` uses the exact ordinary sequential controller with a 2× cap and twelve independent seeds; grouping seeds 1--4, 5--8, and 9--12 gives three homogeneous observed $N=4,K=2$ trials. It has no special dispatch or prompt path.
...
- **Two-block replication:** `baseline-sequential-2x` records one true 2× trajectory and its 1× cut for each of seeds 1–12.
