...
- **parallel** — three replicated eight-run banks (`baseline-parallel`, `hint-parallel`). In bank seed `s`, `run_01` references the frozen matching 1× seed `s`; `run_02`–`run_08` are seven fresh independent 1× attempts. Each bank is exactly 8× and the three bank outcomes support the same ≥2/3 reliability rule as other arms.
...
Single source of experiment knobs: `model` (solver), `audit_model` (judge; must differ from the solver) — both overridable per invocation with `--model` / `--audit-model` (config values are the defaults; the solver≠judge check applies to the effective pair, and audit's `--model` selects whose results tree to grade), `effort` (fixed `high` per the paper), `unit_output_tokens` (1× = 200k), the Uniform Strategy planner/reserve/branch settings, `max_turns_per_phase` / `audit_max_turns` (per-phase guards; the token budget is the sequential stop), `max_concurrency`, and the arm table (`hint`: none/h1/h2/h3, `mode`, `budget_units`, `seeds`). Models are Anthropic ids (`claude-opus-4-8`), OpenRouter `vendor/model` ids (`openai/gpt-5.4`), local Codex-subscription aliases (`litellm/gpt-5.4`), or native Anthropic-compatible vLLM ids (`vllm/qed-nano`). Local routes use their `*_BASE_URL*` values as the round-robin/cooldown pool and a shared `*_API_KEY`; setup is in `docs/codex-subscription-via-litellm.md` and `docs/qed-nano-vllm.md`. Results paths use the model id with `/` replaced by `-`. Arm names are the slugs used everywhere — CLI, `results/` paths, and the top-level README arm table. Baseline/hint and their Parallel banks all use seeds 1–3; each bank records its immutable `run_01` source rather than copying it.

## Compute ladder (1×/2×/4×/8×)

- **Parallel:** each of three prespecified banks contains eight ordered independent attempts. `run_01` is the matching frozen 1× result and `run_02`–`run_08` are fresh. Auditing reports realized best-of-prefix outcomes at 1×/2×/4×/8× per bank; reliable coverage is ≥2/3 banks. Ordering is frozen before grades are known.
...
- **Repeats:** every arm uses three independent top-level seeds (1, 2, 3). One Parallel or Uniform bank is one replicate; bank members live under `seed_<s>/run_01` through `run_08`.
...
- `scratch/` — copy of the proof solver's scratch directory. The live path is short and opaque (`/c/w/a1b2c3d4`) so the prompt carries no arm, contest, or seed identity. Bank members live under `run_01`–`run_08`. Parallel `run_01` contains an immutable reference to the matching baseline/hint result; Uniform stores eight fresh executors plus the shared planner log, `strategies.json`, and `plan_scratch/` at the seed root.
- `meta.json` — attempt metadata, standard provider-usage totals, provider session UUIDs, recovery policy/events, token-accounting status, and sequential `termination_reason` (`self_converged` or `token_limit`); written last, so its presence is the completion marker.
