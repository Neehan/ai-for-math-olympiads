...
Concurrency is async (anyio) under capacity limiters: at most `max_concurrency` (config.json) ordinary agent sessions are admitted at once. Parallel-8 banks retain their fixed eight-way internal fan-out; the scheduler admits `floor(max_concurrency / 8)` complete banks concurrently (two at the current limit of 16). Uniform-C retains its original one-bank-at-a-time schedule. Available credentials are assigned round-robin; keys are not per-request concurrency limits, so one healthy key can serve concurrent sessions. Duplicate values under multiple env names are ignored.
