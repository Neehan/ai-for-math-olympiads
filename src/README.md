...
- **selection / selection_no_problem** — selector seeds 1–3 all rank the same frozen proposal-seed-1 four-item set (three compressed proposals plus the exact frozen oracle sketch) under the standard 1× protocol: a 200k output-token cap, offline scratch tools through the 180k working boundary, and one tool-free final-ranking turn within the reserved 20k. Candidate order is randomized independently by selector seed. `selection` includes the problem statement; `selection-no-problem` withholds only that statement to measure provenance leakage. Both arms use the same candidate order within a selector seed. All store the full ranking, exact oracle rank, and whether the top candidate matches the frozen route; an over-cap or missing ranking is a terminal `no_decision`. The primary scientific label is supplied by expert viability review, with single-viable pools analyzed separately.
